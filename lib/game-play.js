...
GamePlay = (function() {
  ...
  self.earlyClosureStarted = false;
  ...
  self.simulateVotes = ()=>{
      ...
      setTimeout(()=>{
        ...
        var inspector = _.find(playersAlive, (p)=>{ return p.role == constants.CHARACTERS.INSPECTOR.role });
        var civilians = _.filter(playersAlive, (p)=>{ return p.role != constants.CHARACTERS.MAFIA.role });
        var mafia = _.filter(playersAlive, (p)=>{ return p.role == constants.CHARACTERS.MAFIA.role });

        var consoleLogVote = (selected, voted)=>{
            console.log(`${voted.role} - ${voted.username} voted for ${selected.username} [${selected.slackUserId}]`);
        };

        console.log("VOTE SIMULATION STARTED...");

        var markTheVoteWrapper = (selected, voted)=>{
            setTimeout(()=>{
                //self.markTheVote(player, selected);
                pub.messageIn(`<@${selected.slackUserId}>`, {
                    name: voted.username
                }, ()=>{}, (msg)=>{
                    self.userMsgQueue.addToQueue({
                        ...
                        attachments: [
                        {
                            ...
                            "title": "Vote simulation response !!!",
                            "text": msg,
                        },
                        ]                      
                    });
        ...
        if(self.isInNight(self.currentRound)){
            // simulate doctor vote
            if(doctor){
                var picked = Math.floor( Math.random() * ( _.size(playersAlive) ) );
                markTheVoteWrapper(playersAlive[picked], doctor);
                consoleLogVote(playersAlive[picked], doctor);
            }

            // simulate inspector vote
            if(inspector){
                var picked = Math.floor( Math.random() * ( _.size(playersAlive) ) );
                markTheVoteWrapper(playersAlive[picked], inspector);
                consoleLogVote(playersAlive[picked], inspector);
            }

            // simulate mafia
            _.each(mafia, (m)=>{
                if(m){
                    var picked = Math.floor( Math.random() * ( _.size(civilians) ) );
                    markTheVoteWrapper(civilians[picked], m);
                    consoleLogVote(civilians[picked], m);
                }
            });
        } else {
            // simulate all players - day time
            _.each(playersAlive, (p)=>{
                var picked = Math.floor( Math.random() * ( _.size(playersAlive) ) );
                markTheVoteWrapper(playersAlive[picked], p);
                consoleLogVote(playersAlive[picked], p);
            });
        }
            ...
            if(!player.host){
                params.userMsgQueue.addToQueue({
                    ...
                    attachments: [
                    {
                        ...
                        "title": "GAME STARTED !",
                        "text": `Host will set the combinations for the game.`,
                        "fields": [],
                        "footer": `Game will proceed in ${HOST_COMBINATION_TIME_LIMIT_SECONDS} seconds...`,
                        "footer_icon": "https://platform.slack-edge.com/img/default_application_icon.png",
                    },
                    ]                           
                });
  ...
  self.startRound = (db, gameCode, round) => {
        ...
        // update the round and last activity time on DB
        db.collection('configurations').update({
            key : "gameHosted"
        },{
            $set: {
                "value.round": self.currentRound,
                "value.lastActivityOn": new Date(),
            }
        }, {}, ()=>{} );
        ...
        if(noOfMafiasAlive == 0){
            ...
            // do a simulation votes round for testing
  ...
  self.endGame = (civiliansWon) => {
    ...
    self.db.collection('gameRecords').find({
        gameCode: gameCode
    }).toArray((err, events)=>{
        var rounds = _.keyBy(events, "round");
        _.each(events, (event)=>{
            rounds[event.round].events = rounds[event.round].events || [];
            rounds[event.round].events.push(event);
        });
        var gameScript = ["-------------- GAME EVENTS -------------"];
        _.each(rounds, (round)=>{
            var isNight = self.isInNight(round.round);
            gameScript.push(`-- ROUND ${round.round} [${isNight?"NIGHT":"DAY"}]`);            
            var actionByRole = "marked to execute";
            _.each(round.events, (event)=>{
                var diedState = event.died?"[DIED] ":"";
                actionByRole = (isNight && event.role==constants.CHARACTERS.DOCTOR.role)?"healed":actionByRole;
                actionByRole = (isNight && event.role==constants.CHARACTERS.INSPECTOR.role)?"inspected":actionByRole;
                actionByRole = (isNight && event.role==constants.CHARACTERS.MAFIA.role)?"wanted to kill":actionByRole;
                const userAction = event.vote?`- ${actionByRole} ${event.vote}`:"- was sleeping tight..";
                gameScript.push(`> ${event.username} [${event.role}] ${diedState}${userAction}`);
            });
        });
        ...
        _.each(self.players, (player)=>{
            self.userMsgQueue.addToQueue({
                ...
                attachments: [
                    {
                        "color": constants.MESSAGE_COLORS.GREEN,
                        "title": "GAME EVENTS - Full list!!!",
                        "text": `\`\`\`${gameScript.join("\n")} \`\`\``,
                        "fields": [],
                        "footer_icon": "https://platform.slack-edge.com/img/default_application_icon.png",
                    },
                ]                      
  ...
  self.endRound = (db, gameCode, round)=>{
    ...
    var playersAlive = _.filter(self.players, (p)=>{ return !p.dead });
      ...
      if(!deadGuy.doctorHealed){
        ...
        self.updatePlayerOnDB(self.db ,gameCode, deadGuy.username, {
            dead: true
        });

        // update game records with vote
        self.db.collection('gameRecords').update({
            gameCode: self.gameCode,
            round: self.currentRound,
            username: deadGuy.username
        },{
            $set: {
                role: deadGuy.role,
                died: true
  ...
  self.findWhosDead = (playersAlive, gameCode)=>{
    ...
    var pickDeadGuyFromVotes = (voters, killPool) => {
      ...
      if(self.isInNight(self.currentRound)){
          ...
          var mafiasAlive = _.filter(playersAlive, (p)=>{ return (p.role==constants.CHARACTERS.MAFIA.role) });
          var nonMafiasAlive = _.filter(playersAlive, (p)=>{ return (p.role!=constants.CHARACTERS.MAFIA.role) });
          var deadGuy = pickDeadGuyFromVotes(mafiasAlive, nonMafiasAlive);
      ...
      return deadGuy ? _.extend(self.players[deadGuy.username], deadGuy) : null;
  ...
  pub.messageIn = function(msg, user, irrelavantCb, cb){
    if(self.players && self.players[user.name]){
  ...
  pub.hostMessages = function(msg, messagedPlayer, irrelavantCb, cb){
      ...
      if(!self.gameInProgress){
        ...
        if(total<=noOfPlayers){
            ...
            if( combination[constants.CHARACTERS.MAFIA.role] && (combination[constants.CHARACTERS.MAFIA.role].count < noOfPlayers/2) ){
                ...
                var combinationStrArr = [];
                _.each(combination, function(char){
                    combinationStrArr.push(`${char.character}:${char.count}`);
                });
    
                cb(`Updated the combinations to - \`${ combinationStrArr.join(", ") }\` `);
            } else {
                cb("Are you trying to gift an obvious win for mafia... more mafias than civilians..");
            }            
  ...
  self.markTheVote = (selectedPlayer, votingPlayer)=>{
    ...
    self.players[votingPlayer.username].voteTime = voteTime;
    ...
    self.db.collection('gameRecords').update({
        gameCode: self.gameCode,
        round: self.currentRound,
        username: votingPlayer.username
      },{
        $set: {
            role: votingPlayer.role,
            vote: selectedPlayer.username,
            voteTime: voteTime
        }
      }, {
    ...
    var secondsRemainning = self.endRoundTimerStartedAt?(  (nowDate.getTime() -  self.endRoundTimerStartedAt ) / 1000  ):0;
    if(!self.earlyClosureStarted && secondsRemainning > LAST_X_SECONDS_FOR_VOTING){
        // if all have voted
        ...
        if(_.size(votersNotVoted)==0){
            ...
            // mark early closure setting
            self.earlyClosureStarted = true;
            ...
            setTimeout(() => {
                // end round
                self.endRound(self.db, self.gameCode, self.currentRound);
                
                // clear early closure setting
                self.earlyClosureStarted = false;
  ...
  pub.mafiaMessages = (msg, messagedPlayer, irrelavantCb, cb) => {
    ...
    if( selectedPlayer && (selectedPlayer.username == messagedPlayer.username || selectedPlayer.role == constants.CHARACTERS.MAFIA.role ) ){
        ...
        // inform other mafias
        if(messagedPlayer.vote != selectedPlayer.username){ // if not re-selecting the same person
  ...
  self.doctorInspectorMessages =  (msg, messagedPlayer, irrelavantCb, cb)=>{
    ...
    if(selectedPlayer && messagedPlayer.role == constants.CHARACTERS.DOCTOR.role && selectedPlayer.username==messagedPlayer.username && messagedPlayer.healedBefore){
        cb(`You can heal yourself only once in a game as the doctor.`);
    } else if(selectedPlayer && messagedPlayer.role == constants.CHARACTERS.INSPECTOR.role && selectedPlayer.username==messagedPlayer.username ){
